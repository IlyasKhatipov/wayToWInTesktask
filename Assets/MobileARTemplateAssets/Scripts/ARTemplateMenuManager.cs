...
public class ARTemplateMenuManager : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Button that changes the color of a selected object.")]
    Button m_ChangeColorButton;

    /// <summary>
    /// Button that changes the color of a selected object.
    /// </summary>
    public Button changeColorButton
    {
        get => m_ChangeColorButton;
        set => m_ChangeColorButton = value;
    }
    ...
    void OnEnable()
    {
        ...
        m_ChangeColorButton.onClick.AddListener(ChangeColorOfFocusedObject); 
    ...
    void OnDisable()
    {
        ...
        m_ChangeColorButton.onClick.RemoveListener(ChangeColorOfFocusedObject);
        m_PlaneManager.planesChanged -= OnPlaneChanged;
    }
    ...
    void Update()
    {
        ...
        if (m_ShowObjectMenu || m_ShowOptionsModal)
        {
            ...
            else
            {
                ...
                m_ChangeColorButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null); 
            }
    ...
    void ChangeColorOfFocusedObject()
{
    var currentFocusedObject = m_InteractionGroup.focusInteractable;
    if (currentFocusedObject != null)
    {
        // Ïîëó÷àåì êîìïîíåíò ColorMaterialPropertyAffordanceReceiver
        var colorReceiver = currentFocusedObject.transform.GetComponent<ColorMaterialPropertyAffordanceReceiver>();
        if (colorReceiver != null)
        {
            // Ñîçäàåì íîâûé öâåò (íàïðèìåð, ñëó÷àéíûé öâåò)
            Color newColor = new Color(Random.value, Random.value, Random.value, 1.0f);
        }
        else
        {
            Debug.LogWarning("ColorMaterialPropertyAffordanceReceiver íå íàéäåí íà îáúåêòå.");
        }
    }
}
